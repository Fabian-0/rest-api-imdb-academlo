...
function createActor(req,res, next) {
  const { first_name, last_name, dob, biography, profile_photo, active } = req.body;
  Actors.create({ first_name, last_name, dob, biography, profile_photo, active })
    .then(response => {
      console.log(response);
      res.status(201);
      res.json({
...
function updateActor(req,res, next) {
  ...
  Actors.update(dataUpdate, { where: { id } })
    .then(response => {
      console.log(response);
      let resMessage = null;
      let status = null;
